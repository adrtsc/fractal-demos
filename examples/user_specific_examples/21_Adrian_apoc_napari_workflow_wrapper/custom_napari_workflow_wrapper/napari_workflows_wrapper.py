    ...
    *,
    ...
    for i_ROI, indices in enumerate(list_indices):
        ...
        for ind_output, output_name in enumerate(list_outputs):
            ...
            logger.info(
                f"mask is of type {type(mask)}, casting to np.array")

            mask = np.array(mask, dtype='uint32')

            # label connected components. Background is defined as 1
            # because apoc does not allow using 0 as background label
            mask = morphology.label(mask, background=1).astype('uint32')
